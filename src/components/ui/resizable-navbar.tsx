    ...
    >
      {items.map((item, idx) => (
        <Link
          ...
          onClick={onItemClick}
          className="relative px-4 py-2 text-foreground font-medium"
          key={`link-${idx}`}
          href={item.link}
        >
          {hovered === idx && (
...
export const NavbarButton = ({
  ...
  const baseStyles =
    'px-4 py-2 rounded-md bg-primary button bg-white text-black text-sm font-bold relative cursor-pointer hover:-translate-y-0.5 transition duration-200 inline-block text-center'
